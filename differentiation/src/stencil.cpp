...
double approxStencil3(const Callable& F, double x, double y, double step = 1e-4) {
...
template<typename Callable, Derivative D>
double approxStencil5(const Callable& F, double x, double y, double step = 1e-4) {
    double hx = step, hy = step;
    if (std::abs(x) > 1) {
        hx *= std::abs(x);
    }
...
double approxStencilExtra(const Callable& F, double x, double y, double step = 1e-4, int n = 10) {
    assert(n % 2 == 0);
