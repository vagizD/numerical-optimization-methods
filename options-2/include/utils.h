...
template<const size_t n>
void fill_index_matrix(const size_t i, const size_t j, std::array<std::array<size_t, 3>, 3> &a_im) {
...
void update_rlhs(
    ...
    for (size_t j = 1; j < n + 1; ++j) {
        ...
        const double diff = std::exp(- a_sp.m_mp.m_d * time) * Sj - std::exp(- a_sp.m_mp.m_r * time) * a_sp.m_K;
    ...
    for (size_t i = 1; i < m + 1; ++i) {
        ...
        const double diff = std::exp(- a_sp.m_mp.m_d * time) * a_sp.m_S_max - std::exp(- a_sp.m_mp.m_r * time) * a_sp.m_K;
    ...
    for (size_t i = 1; i < m; ++i) {
        for (size_t j = 1; j < n; ++j) {
            ...
            tmp = a_sp.m_mp.m_sigma * a_sp.m_mp.m_sigma / 2 * std::pow(Sj, 2 * a_sp.m_mp.m_beta) / (a_sp.m_hs * a_sp.m_hs);
            tmp *= -time;
            a_A[ij][im[1][2]] += tmp;       // (2)
            ...
            tmp = a_sp.m_mp.m_epsilon * a_sp.m_mp.m_epsilon / 2 * Vi / (a_sp.m_hv * a_sp.m_hv);
            tmp *= -time;
            a_A[ij][im[2][1]] += tmp;       // (2)
            a_A[ij][im[1][1]] -= 2 * tmp;   // (2)
            a_A[ij][im[0][1]] += tmp;       // (2)

            tmp = a_sp.m_mp.m_sigma * a_sp.m_mp.m_epsilon * a_sp.m_mp.m_rho * std::pow(Sj, a_sp.m_mp.m_beta) * Vi / (4 * a_sp.m_hs * a_sp.m_hv);
            tmp *= -time;
            a_A[ij][im[2][2]] += tmp;       // (2)
            a_A[ij][im[2][0]] -= tmp;       // (2)
            a_A[ij][im[0][2]] -= tmp;       // (2)
            ...
            if (k == a_sp.m_p) {
                // NOT a_A_prev[ij][ij] since it is a coef of Cij, we need previous value of Cij
                tmp = a_b_prev[ij];
                a_b[ij] += tmp;             // (3)
            }
        }
...
template<const size_t m, const size_t n>
    ...
    std::array<std::array<double, N>, N> b_prev;
