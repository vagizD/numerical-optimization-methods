...
void fill_index_matrix(
    ...
    for (int dx = -1; dx <= 1; ++dx) {
        for (int dy = -1; dy <= 1; ++dy) {
            a_im[1 + dx][1 + dy] = map2to1<n + 1>(i + dx, j + dy);
        }
    }
}
...
void init_rlhs(
    ...
    for (size_t j = 1; j < n; ++j) {
        const size_t mj = map2to1<n + 1>(m, j);
        const size_t mmj = map2to1<n + 1>(m - 1, j);
    ...
    for (size_t i = 1; i < m; ++i) {
        for (size_t j = 1; j < n; ++j) {
            ...
            const size_t ij = map2to1<n + 1>(i, j);
            const double Sj = S<n>(j, a_sp.m_S_max);
            ...
            tmp = a_sp.m_mp.m_sigma * a_sp.m_mp.m_sigma / 2 *
                  std::pow(Sj, 2 * a_sp.m_mp.m_beta) * Vi / (a_sp.m_hs * a_sp.m_hs);
            tmp *= -tau;
            a_A.add(ij, im[1][2], tmp);       // (2)
